...
export class AuthenticationControllerService extends BaseService {
  ...
  static readonly RegisterPath = '/bank/auth/register';
  ...
  static readonly AuthenticatePath = '/bank/auth/authenticate';
  ...
  authenticate(params: Authenticate$Params, context?: HttpContext): Observable<AuthenticationResponse> {
    // Modify to include the token in the Authorization header
    const token = 'your_token_here'; // Get the token from your token service
    const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);

    return this.authenticate$Response(params, context).pipe(
      map((r: StrictHttpResponse<AuthenticationResponse>): AuthenticationResponse => r.body),
      // You can also pass headers to the request
      // Example: return authenticate(this.http, this.rootUrl, params, { headers }, context);
    );
  }
}
